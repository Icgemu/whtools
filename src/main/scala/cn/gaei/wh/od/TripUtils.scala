...
object TripUtils extends Serializable{
  ...
  def getTripUuid(last:Segment, cur:Segment, mergeLoss:Boolean):Int = {
    ...
    if (s2 == 1) {//od start,new uuid
      ...
      val is_data_loss = if(s1 == 2 && avg_speed > 10) true else false

      if ((ts != t2)) {
        if (is_data_loss && mergeLoss) {
          res = cnt //merge loss;
        } else {
          res = cnt + 1 //not data loss, uuid add one
        }
      } else {
        res = cnt //magic for window duplicated call;
      }
