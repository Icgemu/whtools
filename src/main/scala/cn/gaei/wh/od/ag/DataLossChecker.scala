...
object DataLossChecker {
  ...
  def main(args: Array[String]): Unit = {
    ...
    data
      ...
      .markLoss("loss",$"vin", $"ts", $"keyst", $"icm_totalodometer")
      .groupBy($"date_str").agg(sum($"loss").as("loss_cnt"))
//      .select($"date_str",from_unixtime($"ts"/1000,"yyyy-MM-dd HH:mm:ss"),$"loc_lon84",$"loc_lat84",$"icm_totalodometer",$"bcm_keyst", $"keyst",$"loss")
      .sort($"date_str")
